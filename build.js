...
// Tier D: Inspect + Robo + Automator JS API (~110KB)
// NOTE: this tier exposes window.DK.robo + window.DK.automator as a JS
// API only. The Automator UI panel (ui/automator-panel.js) lives in the
// full tier — it depends on ui/app-shell.js, which itself pulls every
// other panel module. For the UI experience use --tier=full; for
// programmatic / REPL-driven automation, this tier is enough.
...
// Tier E: Field Kit (~125KB) — Inspect + Data + Robo + Automator JS API + Classification
// Same UI caveat as ROBO above: API only, full tier for the panel.
...
// Vendor libraries (loaded before src modules)
...
for (var a = 2; a < process.argv.length; a++) {
  ...
  if (process.argv[a] === '--snippet') {
    // Snippet mode = full tier, no minification, paste-and-run header,
    // skip bookmarklet artifacts (the source IS the deliverable).
    snippetMode = true;
    tier = 'full';
  }
}
...
// Include vendor libs for tiers that need compression/parquet support
// (full tier and data-related tiers)
var includeVendor = (tier === 'full' || tier === 'data' || tier === 'field');
if (includeVendor && fs.existsSync(VENDOR_DIR)) {
  for (var vi = 0; vi < VENDOR_FILES.length; vi++) {
    var vendorPath = path.join(VENDOR_DIR, VENDOR_FILES[vi]);
    if (fs.existsSync(vendorPath)) {
      var vendorCode = fs.readFileSync(vendorPath, 'utf8');
      parts.push('// --- vendor/' + VENDOR_FILES[vi] + ' ---\n\n' + vendorCode);
      console.log('  vendor/' + VENDOR_FILES[vi] + ' (' + (vendorCode.length / 1024).toFixed(1) + ' KB)');
    } else {
      console.warn('  SKIP vendor (not found): ' + VENDOR_FILES[vi]);
    }
  }
}
...
// Snippet artifact — readable, commented, paste into DevTools Sources > Snippets
if (snippetMode) {
  var snippetHeader = [
    '/* ============================================================',
    ' * WDK — paste into Chrome/Edge DevTools Sources > Snippets',
    ' * ============================================================',
    ' *',
    ' * Setup (once per machine):',
    ' *   1. Open DevTools (F12).',
    ' *   2. Sources tab > Snippets pane (left sidebar; if not visible,',
    ' *      use the >> menu or Cmd/Ctrl+Shift+P > "Show Snippets").',
    ' *   3. New snippet > paste this file.',
    ' *',
    ' * Run:',
    ' *   - Cmd/Ctrl+Enter inside the snippet editor, OR',
    ' *   - Right-click the snippet name > "Run".',
    ' *',
    ' * Edit in place:',
    ' *   The whole codebase is below, organized as concatenated modules.',
    ' *   Find anything by Cmd/Ctrl+F. Modules are marked with',
    ' *   "// === <module-path> ===" headers. Edit + Cmd/Ctrl+Enter to',
    ' *   re-run; the panel re-mounts.',
    ' *',
    ' * Iframe note:',
    ' *   WDK\'s network capture and replay engine pierce into',
    ' *   same-origin iframes automatically. Cross-origin iframes are',
    ' *   skipped (browser-enforced).',
    ' *',
    ' * Build: ' + new Date().toISOString() + ' (full tier, unminified)',
    ' * ============================================================ */',
    '',
    ''
  ].join('\n');
  var snippetPath = path.join(DIST, 'wdk-snippet.js');
  fs.writeFileSync(snippetPath, snippetHeader + iife, 'utf8');
  console.log('  -> ' + snippetPath + ' (' + (snippetHeader.length + iife.length) + ' bytes, snippet)');
}
...
if (tier === 'full') {
  var loader = "javascript:void(document.head.appendChild(Object.assign(document.createElement('script'),{src:prompt('WDK URL:','https://example.com/wdk.js')})))";
